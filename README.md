...
To use the FTL, you need a C++14 supporting compiler, such as gcc-5+ and clang-3.8+. You also need an appropriate standard library with similar support.
...
```cpp
ftl::sum_type<std::string,int> x{ftl::type<int>, 3};
```
Here, `x`, can take on any value that is either of type `std::string` or `int` and we've initialised it to be the integer value `3`. It is necessary to use the `type<T>` tag to specify what type to initialise to, because some types may have constructors accepting similar or even the same arguments.
...
Note that `match` doesn't have to return a value, though it is encouraged to do so if adopting a functional style. For example, here's an example of a "dirty" match expression:
```cpp
x.match(
    [](int& x){ ++x; },
...
It can sometimes be a bit boiler-plate heavy to write out every match case explicitly, especially if we're only really interested in one or a couple. Enter `match_all`:
```cpp
...
auto r = x.match(
    ...
    [](match_all){ return g(); }    // This function will be invoked if x's
                                    // value is of any type except B
);
```
It is of course possible to have several specific match clauses before the match-all-clause, but make sure to always put `match_all` last&mdash;or else any clause appearing below it will never actually be executed. Simply put, matching is done in the order the expressions appear.

The true usefulness of `sum_type` isn't so much the case where you use it directly. It's when you use it to quickly and cleanly create new data types. For example, did you know both `ftl::maybe` and `ftl::either` are really just almost empty types inheriting `sum_type`? This is the entire implementation of `maybe`:
```cpp
template<class T>
...
public:
	...
	constexpr maybe(nothing_t)
		: sum_type<T,nothing_t>(type<nothing_t>) noexcept {}
};
```
There are a number of convenience functions and definitions in addition, but that is essentially all there is to `maybe`.
...
for(auto m : *ms) {
    m.match(
        [](int x){ std::cout << x << ", "; },
        [](ftl::nothing_t){ std::cout << "nothing, "; }
    );
}
std::cout << std::endl;
```
...
auto ns = [](maybe<int> x){
    return x.match(
        [](int x){ return ftl::just(x-1); },
        [](match_all){ return ftl::nothing; }
    );
} % ms;
